/* ==========================================================================
   URBAN RUSH: METRO ESCAPE — cinematic endless runner, WebGL engine (Three.js)
...
const CONFIG = {
  ...
  COIN_SCORE: 25,
  POWERUP_DUR: { magnet: 8, shield: 10, x2: 10, boost: 4, slow: 5 },
};
...
class AudioManager {
  constructor() {
    this.ctx = null;
    this.muted = localStorage.getItem("urbanrush_muted") === "1";
    this.musicOn = localStorage.getItem("urbanrush_music") !== "0";
  ...
  setMuted(m) {
    this.muted = m;
    localStorage.setItem("urbanrush_muted", m ? "1" : "0");
    if (m) this.stopMusic();
  }

  setMusic(on) {
    this.musicOn = on;
    localStorage.setItem("urbanrush_music", on ? "1" : "0");
    if (!on) this.stopMusic();
  }

  click() { this.tone({ type: "triangle", from: 700, to: 520, dur: 0.06, vol: 0.12 }); }
  ...
  startMusic() {
    if (this.muted || !this.musicOn || !this.ensure() || this.musicTimer) return;
...
class InputManager {
  constructor(target, handlers) {
    this.h = handlers;
    this.threshold = () => 24; // seuil de swipe (px), remplacé par le jeu
    this.touch = null;
    this.consumed = false;
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
// kind: jump (sauter), slide (glisser), block (changer de voie), train (long)
const OBSTACLE_DEFS = {
  barrier: { kind: "jump",  w: 2.0, h: 1.0,  d: 0.5 },   // barrière de maintenance
  divider: { kind: "jump",  w: 2.1, h: 0.85, d: 0.8 },   // séparateur béton
  pipe:    { kind: "jump",  w: 2.2, h: 0.65, d: 0.7 },   // tuyau tombé
  cables:  { kind: "slide", w: 2.1, h: 1.1,  d: 0.5, gapBottom: 1.22 }, // faisceau de câbles
  duct:    { kind: "slide", w: 2.2, h: 1.05, d: 1.4, gapBottom: 1.26 }, // gaine de ventilation
  cart:    { kind: "block", w: 1.9, h: 1.65, d: 2.4 },   // chariot de service
  pallets: { kind: "block", w: 1.9, h: 1.5,  d: 1.6 },   // palettes chargées
  reel:    { kind: "block", w: 1.8, h: 1.85, d: 1.1 },   // touret de câble
  wagon:   { kind: "train", w: 2.3, h: 3.0,  d: 13 },    // rame à l'arrêt
};
const JUMP_TYPES = ["barrier", "divider", "pipe"];
const SLIDE_TYPES = ["cables", "duct"];
const BLOCK_TYPES = ["cart", "pallets", "reel"];
...
const POWERUP_DEFS = {
  magnet: { color: 0x00d0ff, css: "#00d0ff", label: "M",  name: "Magnet Field" },
  shield: { color: 0x7fd4ff, css: "#7fd4ff", label: "S",  name: "Impact Shield" },
  x2:     { color: 0x8b5cf6, css: "#8b5cf6", label: "x2", name: "Score Doubler" },
  boost:  { color: 0xff8a2a, css: "#ff8a2a", label: "H",  name: "Hyper Sprint" },
  slow:   { color: 0x7ec8ff, css: "#7ec8ff", label: "◔",  name: "Slow Focus" },
};
  ...
  patterns() {
    ...
    return {
      singleJump: (z, free, others) => {
        ...
        G.obstacles.push(new Obstacle(pick(JUMP_TYPES), lane, z + 4));
        coinJumpArc(lane, z);
        coinRow(free, z + 1, 4);
        return 14;
      },
      singleSlide: (z, free, others) => {
        const lane = pick(others);
        G.obstacles.push(new Obstacle(pick(SLIDE_TYPES), lane, z + 4));
        coinRow(lane, z + 1, 5, 2.2, 0.75);
        return 13;
      },
      singleCrate: (z, free, others) => {
        G.obstacles.push(new Obstacle(pick(BLOCK_TYPES), pick(others), z + 3));
        coinRow(free, z, 5);
        return 12;
      },
      coinLine: (z, free) => {
      ...
      doubleBlock: (z, free, others) => {
        G.obstacles.push(new Obstacle(pick(BLOCK_TYPES), others[0], z + 3));
        G.obstacles.push(new Obstacle(pick(Math.random() < 0.5 ? JUMP_TYPES : SLIDE_TYPES), others[1], z + 3));
        coinRow(free, z + 1, 5);
        return 14;
      },
      ...
      jumpThenSlide: (z, free, others) => {
        ...
        G.obstacles.push(new Obstacle(pick(JUMP_TYPES), lane, z + 3));
        G.obstacles.push(new Obstacle(pick(SLIDE_TYPES), lane, z + 3 + step));
        coinRow(free, z + 2, 6);
        return 6 + step + 6;
      },
      triplet: (z, free, others) => {
        ...
        G.obstacles.push(new Obstacle(pick(BLOCK_TYPES), l1, z + 2));
        const free2 = pick([free, ...others.filter((l) => l !== l1)]);
        const blocked2 = [0, 1, 2].filter((l) => l !== free2);
        G.obstacles.push(new Obstacle(pick(JUMP_TYPES), pick(blocked2), z + 2 + step));
        G.obstacles.push(new Obstacle(pick(SLIDE_TYPES), pick(blocked2), z + 2 + step * 2));
      ...
      zigzag: (z, free, others) => {
        ...
        G.obstacles.push(new Obstacle(pick(BLOCK_TYPES), others[0], z + 2));
        G.obstacles.push(new Obstacle(pick(BLOCK_TYPES), others[1], z + 2 + step));
        coinRow(free, z, 3);
      ...
      gauntlet: (z, free, others) => {
        ...
        const seq = [pick(JUMP_TYPES), pick(SLIDE_TYPES), pick(BLOCK_TYPES), pick(JUMP_TYPES)];
...
class CollisionManager {
  static check(player, obstacles) {
    ...
    for (const o of obstacles) {
      ...
      return o; // block / train : infranchissable
    }
    return null;
  }
}
...
class ScoreManager {
  constructor() {
    this.best = parseInt(localStorage.getItem("urbanrush_best") || "0", 10);
    this.reset();
  }
  reset() {
    this.score = 0;
  ...
  finalize() {
    ...
    if (s > this.best) {
      this.best = s;
      this.newBest = true;
      localStorage.setItem("urbanrush_best", String(s));
    }
    return s;
  }
  resetBest() {
    this.best = 0;
    localStorage.removeItem("urbanrush_best");
  }
}
...
class UIManager {
  ...
  updateHud(score, coins, best, dist) {
    this.$("hud-score").textContent = Math.floor(score);
    this.$("hud-coins").textContent = coins;
    this.$("hud-best").textContent = best;
    this.$("hud-distance").textContent = `${Math.floor(dist)} m`;
  }

  /* Bandeau volant : Perfect Dodge, Near Miss, Combo… */
  toast(text, cls = "") {
  ...
  updatePowerups(active) {
    for (const kind of Object.keys(POWERUP_DEFS)) {
      ...
      if (a && !el) {
        ...
        el.innerHTML = `<span class="pu pu-${kind}">${POWERUP_DEFS[kind].label}</span><div><div class="pu-name">${POWERUP_DEFS[kind].name}</div><div class="pu-bar"><div class="pu-fill" style="background:${POWERUP_DEFS[kind].css}"></div></div></div>`;
  ...
  showGameOver(stats) {
    ...
    this.$("go-dodges").textContent = stats.dodges;
...
const Tex = {
  ...
  wall() {
    return makeTexture(1024, 128, (g, w, h) => {
      ...
      // graffitis abstraits : halos, flèches, coulures — aucune lettre
      const cols = ["#ff5c8a", "#00e08a", "#ffc93c", "#00d0ff", "#c084fc"];
      for (let i = 0; i < 7; i++) {
        const x = rand(40, w - 80), y = rand(h * 0.25, h * 0.7);
        const col = pick(cols);
        g.fillStyle = col + "55";
        g.beginPath();
        g.ellipse(x, y, rand(34, 58), rand(14, 22), rand(-0.2, 0.2), 0, Math.PI * 2);
        g.fill();
        g.strokeStyle = col;
        ...
        g.beginPath();
        const kind = i % 3;
        if (kind === 0) {          // double vague
          g.moveTo(x - 30, y);
          g.quadraticCurveTo(x - 10, y - 16, x + 8, y);
          g.quadraticCurveTo(x + 24, y + 14, x + 40, y - 4);
        } else if (kind === 1) {   // flèche stylisée
          g.moveTo(x - 26, y + 8); g.lineTo(x + 18, y - 8);
          g.moveTo(x + 4, y - 12); g.lineTo(x + 18, y - 8); g.lineTo(x + 12, y + 4);
        } else {                   // anneaux concentriques
          g.arc(x, y, 10, 0, Math.PI * 2);
          g.moveTo(x + 18, y);
          g.arc(x, y, 18, 0, Math.PI * 1.4);
        }
        g.stroke();
        // coulures
        g.strokeStyle = col + "88";
        g.lineWidth = 2.5;
        for (let dcount = 0; dcount < 3; dcount++) {
          const dx = x + rand(-24, 24);
          g.beginPath();
          g.moveTo(dx, y + 8);
          g.lineTo(dx, y + rand(16, 30));
          g.stroke();
        }
      }
    }, { repeat: [10, 1] });
  ...
  /* Enseigne néon abstraite : glyphe lumineux, jamais de texte */
  neon(seed, color) {
    return makeTexture(256, 64, (g, w, h) => {
      g.clearRect(0, 0, w, h);
      const kind = (String(seed).length + (String(seed).charCodeAt(0) || 0)) % 4;
      g.strokeStyle = color;
      g.lineWidth = 7;
      g.lineCap = "round";
      g.shadowColor = color;
      ...
      g.beginPath();
      if (kind === 0) {          // éclair
        g.moveTo(cx - 26, cy - 20); g.lineTo(cx + 2, cy - 2);
        g.lineTo(cx - 10, cy + 2); g.lineTo(cx + 26, cy + 20);
      } else if (kind === 1) {   // trois anneaux
        for (const dx of [-40, 0, 40]) { g.moveTo(cx + dx + 14, cy); g.arc(cx + dx, cy, 14, 0, Math.PI * 2); }
      } else if (kind === 2) {   // onde
        g.moveTo(cx - 60, cy);
        for (let x = -60; x <= 60; x += 10) g.lineTo(cx + x, cy + Math.sin(x / 14) * 14);
      } else {                   // losange + barre
        g.moveTo(cx, cy - 20); g.lineTo(cx + 22, cy); g.lineTo(cx, cy + 20); g.lineTo(cx - 22, cy); g.closePath();
        g.moveTo(cx - 52, cy); g.lineTo(cx - 32, cy);
        g.moveTo(cx + 32, cy); g.lineTo(cx + 52, cy);
      }
      g.stroke();
      g.shadowBlur = 0;
      g.globalAlpha = 0.85;
  ...
  /* Panneau publicitaire abstrait : formes et lumière, pas de texte */
  billboard(seed) {
    return makeTexture(512, 160, (g, w, h) => {
      const hue = (String(seed).length * 47) % 360;
      const grad = g.createLinearGradient(0, 0, w, h);
      grad.addColorStop(0, `hsl(${hue}, 60%, 16%)`);
      grad.addColorStop(1, `hsl(${(hue + 60) % 360}, 55%, 26%)`);
      g.fillStyle = grad;
      g.fillRect(0, 0, w, h);
      // produit stylisé : cercle lumineux + traînée
      const cx = w * 0.3, cy = h / 2;
      const halo = g.createRadialGradient(cx, cy, 6, cx, cy, 60);
      halo.addColorStop(0, "rgba(255,255,255,0.95)");
      halo.addColorStop(0.4, `hsla(${(hue + 180) % 360}, 90%, 70%, 0.8)`);
      ...
      for (let i = 0; i < 3; i++) {
        const bw = 90 - i * 22;
        g.beginPath();
        g.roundRect(w * 0.52, h * 0.3 + i * 26, bw + Math.sin(i) * 10, 12, 6);
        g.fill();
      }
  ...
  /* Caisse de fret : chevrons de danger et éraflures, aucune lettre */
  crate(base) {
    return makeTexture(256, 256, (g, w, h) => {
      ...
      g.lineWidth = 10;
      g.strokeRect(5, 5, w - 10, h - 10);
      // bande de danger diagonale
      g.save();
      g.beginPath();
      g.rect(0, h * 0.4, w, h * 0.2);
      g.clip();
      for (let x = -h; x < w + h; x += 42) {
        g.fillStyle = (x / 42) % 2 ? "#d9a92e" : "#181820";
        g.beginPath();
        g.moveTo(x, h * 0.6); g.lineTo(x + 21, h * 0.6);
        g.lineTo(x + 21 + 24, h * 0.4); g.lineTo(x + 24, h * 0.4);
        g.closePath(); g.fill();
      }
      g.restore();
      // éraflures et usure
      ...
      for (let i = 0; i < 8; i++) {
        ...
        g.moveTo(x, y); g.lineTo(x + rand(-30, 30), y + rand(-16, 16));
        g.stroke();
      }
      for (let i = 0; i < 14; i++) {
        g.fillStyle = `rgba(0,0,0,${rand(0.08, 0.2)})`;
        g.beginPath();
        g.ellipse(rand(0, w), rand(0, h), rand(6, 20), rand(3, 9), rand(0, 3), 0, Math.PI * 2);
        g.fill();
      }
    });
  },

  /* Signal lumineux « passez dessous » : chevrons, aucun texte */
  signPanel() {
    return makeTexture(512, 128, (g, w, h) => {
      ...
      g.shadowBlur = 16;
      for (const cx of [w * 0.25, w * 0.5, w * 0.75]) {
        for (const cy of [h * 0.34, h * 0.62]) {
          g.beginPath();
  ...
  /* Enseigne suspendue de la station : symbole métro, pas de texte */
  stationSign() {
    return makeTexture(512, 96, (g, w, h) => {
      ...
      g.shadowBlur = 12;
      // symbole : cercle + double flèche de transit
      g.strokeStyle = "#ffe9a8";
      g.lineWidth = 7;
      g.beginPath();
      g.arc(w * 0.5, h / 2, 26, 0, Math.PI * 2);
      g.stroke();
      g.fillStyle = "#ffe9a8";
      g.beginPath();
      g.moveTo(w * 0.5 - 12, h / 2 + 10); g.lineTo(w * 0.5 - 12, h / 2 - 10); g.lineTo(w * 0.5 - 2, h / 2 + 2);
      g.lineTo(w * 0.5 + 8, h / 2 - 10); g.lineTo(w * 0.5 + 8, h / 2 + 10);
      g.lineTo(w * 0.5 - 2, h / 2 - 2);
      g.closePath();
      g.fill();
      // tirets latéraux
      for (const sd of [-1, 1]) {
        g.fillRect(w * 0.5 + sd * 60 - 20, h / 2 - 4, 40, 8);
        g.fillRect(w * 0.5 + sd * 120 - 14, h / 2 - 4, 28, 8);
      }
    });
  },
...
class PlayerRig {
  ...
  /* trébuchement lorsque le bouclier absorbe un impact */
  stumble() { this.stumbleT = 0.55; }

  /* mode : "run" | "idle" | "dead" | "celebrate" — deadK = progression de la chute */
  update(player, dt, fx, speedK = 0, mode = "run", deadK = 0) {
    const g = this.group;
    ...
    this.slideW = damp(this.slideW, mode === "run" && player.sliding ? 1 : 0, 14, dt);
    this.airW = damp(this.airW, mode === "run" && player.jumping ? 1 : 0, 12, dt);
    this.idleW = damp(this.idleW ?? 1, mode === "idle" ? 1 : 0, 5, dt);
    this.celebW = damp(this.celebW ?? 0, mode === "celebrate" ? 1 : 0, 5, dt);
    if ((this.stumbleT ?? 0) > 0) this.stumbleT -= dt;
    const stK = clamp((this.stumbleT ?? 0) / 0.55, 0, 1);
    const stJerk = Math.sin((1 - stK) * Math.PI) * (stK > 0 ? 1 : 0);

    const slideW = this.slideW, airW = this.airW;
    const idleW = this.idleW, celebW = this.celebW;
    const ground = (1 - airW) * (1 - slideW);
    const liveW = 1 - deadK;

    const t = player.runPhase;
    const bob = Math.abs(Math.cos(t)) * 0.05 * ground * (1 - idleW) * liveW;
    const hop = celebW * Math.abs(Math.sin(now / 240)) * 0.1;
    g.position.y = (player.y + bob - slideW * 0.55) * liveW + hop;

    /* chute au game over : bascule avant + affaissement */
    g.rotation.x = -deadK * 1.35;
    g.position.y -= deadK * 0.12;

    /* respiration */
    const breath = 1 + Math.sin(now / 850) * (0.022 + idleW * 0.012);
    this.chest.scale.set(1.14 * breath, 1, 0.86 * breath);

    /* squash d'atterrissage */
    if (this.squashT > 0) {
      this.squashT -= dt;
    ...
    /* inclinaison dans les virages (course uniquement) */
    const lean = clamp((CONFIG.LANE_X[player.lane] - player.x) * 0.4, -0.35, 0.35) * (1 - idleW) * liveW;
    g.rotation.z = damp(g.rotation.z, -lean, 12, dt);
    g.rotation.y = damp(g.rotation.y, lean * 0.5, 10, dt);

    /* ---- poses : course (avec air/glissade), repos, célébration ---- */
    const legCycle = (p) => ({
    ...
    const mixRun = (run, air, slide) => lerp(lerp(run, air, airW), slide, slideW);
    const F = (run, idle, celeb) => lerp(lerp(run, idle, idleW), celeb, celebW);

    const cL = legCycle(t), cR = legCycle(t + Math.PI);
    const idleShift = Math.sin(now / 2100) * 0.05;      // report de poids
    this.legL.hip.rotation.x = F(mixRun(cL.thigh, airL.thigh, slideL.thigh), 0.03 + idleShift, 0.05);
    this.legL.kn.rotation.x = F(mixRun(cL.shin, airL.shin, slideL.shin), 0.08, 0.1);
    this.legL.foot.rotation.x = F(mixRun(cL.foot, airL.foot, slideL.foot), -0.05, -0.05);
    this.legR.hip.rotation.x = F(mixRun(cR.thigh, airR.thigh, slideR.thigh), 0.03 - idleShift, 0.05);
    this.legR.kn.rotation.x = F(mixRun(cR.shin, airR.shin, slideR.shin), 0.08, 0.1);
    this.legR.foot.rotation.x = F(mixRun(cR.foot, airR.foot, slideR.foot), -0.05, -0.05);
    ...
    /* bras : course / repos détendu / poings levés (+ moulinets si trébuchement) */
    ...
    const pump = -2.55 + Math.sin(now / 180) * 0.22;    // célébration
    this.armL.sh.rotation.x = F(mixRun(armSwing, -2.4, -0.5), 0.12, pump) + stJerk * 1.3;
    this.armR.sh.rotation.x = F(mixRun(-armSwing, -2.1, -0.7), -0.05, pump + 0.15) + stJerk * 1.0;
    this.armL.el.rotation.x = F(mixRun(elbowRun, -0.4, -0.5), -0.35, -0.25);
    this.armR.el.rotation.x = F(mixRun(elbowRunR, -0.5, -0.4), -0.3, -0.3);
    this.armL.sh.rotation.z = 0.12 + slideW * 0.5 + celebW * 0.35 + stJerk * 0.4;
    this.armR.sh.rotation.z = -0.12 - slideW * 0.5 - celebW * 0.35 - stJerk * 0.5;

    /* torse + tête : penché en course, regard qui balaie au repos */
    const runPitch = mixRun(0.14 + speedK * 0.12, -0.02, -1.3);
    this.pelvisG.rotation.x = F(runPitch, 0.02, -0.05) + stJerk * 0.45;
    this.torso.rotation.y = Math.sin(t) * 0.1 * ground * (1 - idleW) * liveW;
    this.headG.rotation.x = -this.pelvisG.rotation.x * 0.55 + Math.sin(now / 1400) * 0.03 - celebW * 0.2;
    this.headG.rotation.y = -this.torso.rotation.y * 0.6
      + Math.sin(now / 2300) * 0.05
      + idleW * Math.sin(now / 2600) * 0.35;
    this.pelvisG.position.x = idleW * Math.sin(now / 2100) * 0.04;

    /* sac à dos : inertie secondaire (rebond de foulée + retard de saut) */
    const packTarget = -runPitch * 0.35
      + Math.sin(t * 2) * 0.05 * ground * (1 - idleW)
      - clamp(player.vy * 0.025, -0.3, 0.3);
    this.pack.rotation.x = damp(this.pack.rotation.x, packTarget, 10, dt);

    /* ombre d'appoint */
    this.shadowMesh.position.set(player.x, 0.02, -CONFIG.PLAYER_Z);
    const sh = clamp(1 - player.y / 3, 0.3, 1) * liveW;
    this.shadowMesh.scale.set(sh || 0.001, (sh || 0.001) * (1 + slideW * 0.7), 1);
...
class World3D {
  constructor(scene, glowTex) {
    ...
    /* lampes suspendues + faisceaux poussiéreux + vacillement */
    this.tunnelLamps = [];
    for (let i = 0; i < this.TUNNEL_LEN / 12; i++) {
      const lampMat = new THREE.MeshBasicMaterial({ color: 0xffe9b8 });
      const lampT = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.1, 0.2), lampMat);
      ...
      /* cône de lumière volumétrique */
      const beam = new THREE.Mesh(
        new THREE.ConeGeometry(1.5, 4.4, 18, 1, true),
        new THREE.MeshBasicMaterial({
          color: 0xffdca0, transparent: true, opacity: 0.06,
          blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide,
        })
      );
      beam.position.set(0, 2.4, lampT.position.z);
      this.tunnel.add(beam);
      this.tunnelLamps.push({ lampMat, glowMat: lglow2.material, beamMat: beam.material, seed: i * 1.7 });
    }
    ...
    /* ---------- train lointain qui traverse la skyline ---------- */
    const bgTrainTex = makeTexture(512, 48, (g, w, h) => {
      g.fillStyle = "#141a2c";
      g.fillRect(0, 0, w, h);
      g.fillStyle = "rgba(255,214,150,0.85)";
      for (let x = 8; x < w - 8; x += 22) g.fillRect(x, h * 0.3, 12, h * 0.34);
      g.fillStyle = "rgba(0,0,0,0.5)";
      for (let x = 110; x < w; x += 128) g.fillRect(x, h * 0.15, 5, h * 0.7);
    });
    this.bgTrain = new THREE.Mesh(
      new THREE.PlaneGeometry(64, 3.4),
      new THREE.MeshBasicMaterial({ map: bgTrainTex, fog: false, transparent: true, opacity: 0.9 })
    );
    this.bgTrain.position.set(-220, 7.5, -152);
    scene.add(this.bgTrain);
    this.bgTrainTimer = rand(6, 14);
    this.bgTrainDir = 1;
  }
  ...
  update(dt, speed) {
    ...
    /* vacillement des lampes de tunnel */
    for (const l of this.tunnelLamps) {
      const f = Math.random() < 0.01 ? rand(0.15, 0.5)
        : 0.75 + Math.sin(now / 90 + l.seed * 7) * 0.08;
      l.lampMat.color.setScalar(f + 0.25);
      l.glowMat.opacity = 0.6 * f;
      l.beamMat.opacity = 0.06 * f;
    }

    /* train qui passe au loin en arrière-plan */
    if (this.bgTrainTimer > 0) {
      this.bgTrainTimer -= dt;
      if (this.bgTrainTimer <= 0) {
        this.bgTrain.position.x = this.bgTrainDir > 0 ? -220 : 220;
        this.bgTrainDir *= -1;
      }
    } else {
      this.bgTrain.position.x += this.bgTrainDir * dt * 55;
      if (Math.abs(this.bgTrain.position.x) > 230) this.bgTrainTimer = rand(18, 40);
    }
...
class Game {
  constructor() {
    ...
    this.fxLevel = { low: 0, med: 1, high: 2 }[localStorage.getItem("urbanrush_fx")] ?? 2;
    this.sens = localStorage.getItem("urbanrush_sens") || "normal";
    ...
    this.shakeT = 0;
    this.dieT = 0;
    this.dodges = 0;         // Perfect Dodges de la course
    this.combo = 0;          // pièces consécutives
    this.comboT = 0;
    ...
    this.input.threshold = () => ({ low: 34, normal: 24, high: 15 }[this.sens] || 24);
    this.bindUI();
  ...
  initPools() {
    ...
    const metalDark = std(0x454b68, 0.55, 0.4);
    const metalLight = std(0x8a93ae, 0.4, 0.6);
    const rubber = std(0x1b1e28, 0.95);
    const wood = std(0x6b5233, 0.9);
    const concrete = std(0x8d8f9a, 0.95);

    /* Matériau partagé des gyrophares de chantier (clignote globalement) */
    this.blinkMat = new THREE.MeshBasicMaterial({ color: 0xffb020 });

    /* ---- SAUTER : barrière de maintenance métallique ---- */
    ...
    const mkBarrier = () => {
      ...
      grp.add(decal);
      for (const sd of [-1, 1]) {
        const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.055, 0.55, 12), metalDark);
        leg.position.set(sd * (D.barrier.w / 2 - 0.15), 0.27, 0);
        grp.add(leg);
        const footPad = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.11, 0.04, 12), metalDark);
    ...
    /* ---- SAUTER : séparateur en béton usé ---- */
    const mkDivider = () => {
      const grp = new THREE.Group();
      const body = new THREE.Mesh(roundedBoxGeo(D.divider.w, D.divider.h, D.divider.d, 0.1), concrete);
      body.position.y = D.divider.h / 2;
      grp.add(body);
      const base = new THREE.Mesh(roundedBoxGeo(D.divider.w + 0.15, 0.16, D.divider.d + 0.15, 0.05), std(0x6f7180, 0.95));
      base.position.y = 0.08;
      grp.add(base);
      // salissure sombre en pied
      const stain = new THREE.Mesh(roundedBoxGeo(D.divider.w + 0.01, 0.22, D.divider.d + 0.01, 0.05), std(0x5d5f6c, 0.95));
      stain.position.y = 0.22;
      grp.add(stain);
      castAll(grp);
      return grp;
    };

    /* ---- SAUTER : gros tuyau industriel tombé sur la voie ---- */
    const mkPipe = () => {
      const grp = new THREE.Group();
      const pipe = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.32, D.pipe.w, 20), metalLight);
      pipe.rotation.z = Math.PI / 2;
      pipe.position.y = 0.33;
      grp.add(pipe);
      for (const sd of [-0.7, 0.7]) {
        const ring = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.045, 10, 22), metalDark);
        ring.rotation.y = Math.PI / 2;
        ring.position.set(sd, 0.33, 0);
        grp.add(ring);
      }
      // cale en bois sous le tuyau
      const wedge = new THREE.Mesh(roundedBoxGeo(0.5, 0.16, 0.5, 0.04), wood);
      wedge.position.set(0.5, 0.08, 0.1);
      grp.add(wedge);
      castAll(grp);
      return grp;
    };

    /* ---- GLISSER : faisceau de câbles qui pend entre deux poteaux ---- */
    const mkCables = () => {
      const grp = new THREE.Group();
      for (const sd of [-1, 1]) {
        const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 2.5, 12), metalDark);
        pole.position.set(sd * D.cables.w / 2, 1.25, 0);
        grp.add(pole);
      }
      // câbles : tores aplatis qui pendent (3 brins)
      for (let c = 0; c < 3; c++) {
        const sag = new THREE.Mesh(new THREE.TorusGeometry(D.cables.w / 2, 0.035 + c * 0.008, 8, 30, Math.PI), rubber);
        sag.rotation.z = Math.PI;                 // arc vers le bas
        sag.position.set(0, 2.4, -0.06 + c * 0.06);
        sag.scale.y = 0.55 + c * 0.08;            // affaissement
        grp.add(sag);
      }
      // gyrophare d'avertissement
      const blink = new THREE.Mesh(new THREE.SphereGeometry(0.06, 10, 8), this.blinkMat);
      blink.position.set(0, D.cables.gapBottom + 0.1, 0.14);
      grp.add(blink);
      castAll(grp);
      return grp;
    };

    /* ---- GLISSER : gaine de ventilation suspendue ---- */
    const ductTex = Tex.signPanel();
    const mkDuct = () => {
      const grp = new THREE.Group();
      const body = new THREE.Mesh(
        roundedBoxGeo(D.duct.w, D.duct.h, D.duct.d, 0.12),
        std(0x9aa3ae, 0.45, 0.55)
      );
      body.position.y = D.duct.gapBottom + D.duct.h / 2;
      grp.add(body);
      // anneaux de renfort
      for (const zz of [-D.duct.d * 0.3, 0.0, D.duct.d * 0.3]) {
        const rib = new THREE.Mesh(roundedBoxGeo(D.duct.w + 0.06, D.duct.h + 0.06, 0.1, 0.05), metalDark);
        rib.position.set(0, D.duct.gapBottom + D.duct.h / 2, zz);
        grp.add(rib);
      }
      // suspentes
      for (const sd of [-1, 1]) {
        const rod = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 1.4, 8), metalDark);
        rod.position.set(sd * (D.duct.w / 2 - 0.2), D.duct.gapBottom + D.duct.h + 0.7, 0);
        grp.add(rod);
      }
      // signal chevrons sous la gaine
      const face = new THREE.Mesh(
        new THREE.PlaneGeometry(D.duct.w - 0.3, 0.4),
        new THREE.MeshBasicMaterial({ map: ductTex, transparent: true })
      );
      face.position.set(0, D.duct.gapBottom + 0.22, D.duct.d / 2 + 0.06);
      grp.add(face);
      castAll(grp);
      return grp;
    };

    /* ---- CHANGER DE VOIE : chariot de maintenance ---- */
    const mkCart = () => {
      const grp = new THREE.Group();
      const bed = new THREE.Mesh(roundedBoxGeo(D.cart.w, 0.5, D.cart.d, 0.08), std(0x3f4a63, 0.6, 0.3));
      bed.position.y = 0.55;
      grp.add(bed);
      const rail1 = new THREE.Mesh(roundedBoxGeo(D.cart.w, 0.5, 0.08, 0.03), metalDark);
      rail1.position.set(0, 1.1, -D.cart.d / 2 + 0.06);
      grp.add(rail1);
      const rail2 = rail1.clone();
      rail2.position.z = D.cart.d / 2 - 0.06;
      grp.add(rail2);
      // chargement bâché
      const load = new THREE.Mesh(roundedBoxGeo(D.cart.w - 0.4, 0.7, D.cart.d - 0.9, 0.2), std(0x7a4b2a, 0.9));
      load.position.y = 1.15;
      grp.add(load);
      for (const zz of [-D.cart.d / 2 + 0.45, D.cart.d / 2 - 0.45]) {
        for (const sd of [-1, 1]) {
          const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.08, 14), rubber);
          wheel.rotation.z = Math.PI / 2;
          wheel.position.set(sd * (D.cart.w / 2 - 0.18), 0.18, zz);
          grp.add(wheel);
        }
      }
      castAll(grp);
    ...
    /* ---- CHANGER DE VOIE : pile de palettes chargées ---- */
    const crateTexs = [Tex.crate("#8b5cf6"), Tex.crate("#4a5178"), Tex.crate("#37538a")];
    const mkPallets = () => {
      const grp = new THREE.Group();
      for (let p = 0; p < 3; p++) {
        const pal = new THREE.Mesh(roundedBoxGeo(D.pallets.w, 0.13, D.pallets.d, 0.03), wood);
        pal.position.y = 0.1 + p * 0.16;
        pal.rotation.y = (p - 1) * 0.08;
        grp.add(pal);
      }
      const crate = new THREE.Mesh(roundedBoxGeo(D.pallets.w - 0.3, 0.85, D.pallets.d - 0.3, 0.08), std(0x556084, 0.8));
      crate.position.y = 1.0;
      grp.add(crate);
      const decal = new THREE.Mesh(
        new THREE.PlaneGeometry(D.pallets.w - 0.55, 0.68),
        new THREE.MeshLambertMaterial({ map: pick(crateTexs) })
      );
      decal.position.set(0, 1.0, D.pallets.d / 2 - 0.14);
      grp.add(decal);
      castAll(grp);
      return grp;
    };

    /* ---- CHANGER DE VOIE : touret de câble industriel ---- */
    const mkReel = () => {
      const grp = new THREE.Group();
      const r = D.reel.h / 2;
      for (const zz of [-D.reel.d / 2 + 0.08, D.reel.d / 2 - 0.08]) {
        const disc = new THREE.Mesh(new THREE.CylinderGeometry(r, r, 0.12, 26), wood);
        disc.rotation.x = Math.PI / 2;
        disc.position.set(0, r, zz);
        grp.add(disc);
      }
      const drum = new THREE.Mesh(new THREE.CylinderGeometry(r * 0.45, r * 0.45, D.reel.d - 0.3, 18), rubber);
      drum.rotation.x = Math.PI / 2;
      drum.position.y = r;
      grp.add(drum);
      // câble enroulé apparent
      const coil = new THREE.Mesh(new THREE.TorusGeometry(r * 0.55, 0.05, 8, 26), rubber);
      coil.position.set(0, r, 0);
      grp.add(coil);
      castAll(grp);
      return grp;
    };

    /* ---- TRAIN : rame de métro galbée (voir version précédente) ---- */
    const wagonSkins = [
    ...
    this.factories = {
      barrier: mkBarrier, divider: mkDivider, pipe: mkPipe,
      cables: mkCables, duct: mkDuct,
      cart: mkCart, pallets: mkPallets, reel: mkReel,
      wagon: mkWagon,
    };
    this.pools = {};
    for (const k of Object.keys(this.factories)) this.pools[k] = [];
  ...
  bindUI() {
    ...
    const fxBtn = $("btn-fx");
    const sensBtn = $("btn-sens");
    const FX_LABELS = ["Low", "Medium", "High"];
    const SENS_ORDER = ["low", "normal", "high"];
    const syncOptions = () => {
      soundBtn.textContent = this.audio.muted ? "🔇 Sound: Off" : "🔊 Sound: On";
      musicBtn.textContent = this.audio.musicOn ? "🎵 Music: On" : "🎵 Music: Off";
      fxBtn.textContent = `✨ Visual Effects: ${FX_LABELS[this.fxLevel]}`;
      sensBtn.textContent = `📱 Swipe Sensitivity: ${this.sens[0].toUpperCase()}${this.sens.slice(1)}`;
    };
    ...
    musicBtn.addEventListener("click", () => {
      ...
      if (this.audio.musicOn && this.state === "playing") this.audio.startMusic();
      syncOptions();
    });
    fxBtn.addEventListener("click", () => {
      this.fxLevel = (this.fxLevel + 1) % 3;
      localStorage.setItem("urbanrush_fx", ["low", "med", "high"][this.fxLevel]);
      this.applyQuality();
      syncOptions();
    });
    sensBtn.addEventListener("click", () => {
      this.sens = SENS_ORDER[(SENS_ORDER.indexOf(this.sens) + 1) % 3];
      localStorage.setItem("urbanrush_sens", this.sens);
      syncOptions();
    });
    ...
    // petit clic sonore sur tous les boutons
    document.querySelectorAll(".btn, .hud-pause").forEach((b) =>
  ...
  applyQuality() {
    ...
    const cap = [1.15, 1.6, 2][this.fxLevel];
    this.renderer.setPixelRatio(Math.min(dpr, cap));
    this.sun.castShadow = this.fxLevel >= 1;
    if (this.world) this.world.setDetail(this.fxLevel >= 1);
  }
  ...
  get fxHigh() { return this.fxLevel >= 1; }
  ...
  update(dt) {
    ...
    // Slow Focus : le monde ralentit, le coureur reste réactif
    const eff = this.speed * (this.has("slow") && !this.has("boost") ? 0.72 : 1);
    this.effSpeed = eff;

    const mult = this.has("x2") ? 2 : 1;
    this.scoreMgr.addDistance(eff * dt, mult);

    this.player.update(dt);
    this.world.update(dt, eff);
    this.spawner.update(dt, eff, this.elapsed);
    ...
    // Esquives : un obstacle vient de passer le plan du joueur sans contact
    for (const o of this.obstacles) {
      if (o.counted || o.z + o.def.d > CONFIG.PLAYER_Z - 0.6) continue;
      o.counted = true;
      const dx = Math.abs(this.player.x - o.x);
      if (dx < 1.2 && (o.def.kind === "jump" || o.def.kind === "slide")) {
        this.dodges++;
        this.scoreMgr.score += 50 * mult;
        this.ui.toast("PERFECT DODGE +50", "perfect");
      } else if (dx > 1.35 && dx < 2.3 && o.def.kind !== "train") {
        this.scoreMgr.score += 15 * mult;
        this.ui.toast("NEAR MISS +15", "near");
      }
    }

    // Combo de pièces : fenêtre de 1,6 s entre deux collectes
    if (this.comboT > 0) {
      this.comboT -= dt;
    ...
    for (const c of this.coins) {
      ...
      if (Math.abs(c.z - CONFIG.PLAYER_Z) < 1.4 &&
        ...
        if (this.combo > 0 && this.combo % 10 === 0) {
          this.scoreMgr.score += 100 * mult;
          this.ui.toast(`COMBO x${this.combo} +100`);
        }
        this.audio.coin();
    ...
    // Étincelles électriques aléatoires le long des rails
    if (this.fxLevel >= 1 && Math.random() < dt * 1.2) {
      const lane = pick(CONFIG.LANE_X);
      const off = pick([-0.72, 0.72]);
      this.emit({ x: lane + off, y: 0.08, z: -rand(8, 30), count: 6, color: 0xffd27a, speed: 2.2, size: 0.16, life: 0.35, gravity: -6 });
    }

    this.shakeT = Math.max(0, this.shakeT - dt);
    this.ui.updateHud(this.scoreMgr.score, this.scoreMgr.coins, this.scoreMgr.best, this.world.dist);
  ...
  updateCamera(dt) {
    ...
    /* Menu : orbite cinématique lente autour du coureur au repos */
    if (this.state === "menu" || this.state === "loading") {
      const a = now / 7000;
      const tx = Math.sin(a) * 2.6;
      const tz = -CONFIG.PLAYER_Z - 2.6 - Math.abs(Math.cos(a)) * 1.6;
      this.camera.position.x = damp(this.camera.position.x, tx, 2.5, dt);
      this.camera.position.y = damp(this.camera.position.y, 1.45 + Math.sin(now / 4300) * 0.15, 2.5, dt);
      this.camera.position.z = damp(this.camera.position.z, tz, 2.5, dt);
      this.camera.fov = damp(this.camera.fov, this.baseFov - 6, 4, dt);
      this.camera.updateProjectionMatrix();
      this.camera.lookAt(0, 1.05, -CONFIG.PLAYER_Z);
      return;
    }
  ...
  render(dt) {
    ...
    const speedK = clamp((this.speed - CONFIG.BASE_SPEED) / (CONFIG.MAX_SPEED - CONFIG.BASE_SPEED), 0, 1);
    const mode = (this.state === "menu" || this.state === "loading") ? "idle"
      : this.state === "dying" ? "dead"
      : this.state === "gameover" ? "celebrate"
      : "run";
    const deadK = this.state === "dying" ? clamp(1 - this.dieT / 0.9, 0, 1)
      : this.state === "gameover" ? 0 : 0;
    this.syncEntityMeshes();
    this.rig.update(this.player, dt, { shield: this.has("shield"), boost: this.has("boost") }, speedK, mode, deadK);
    ...
    const speedFx = this.state === "playing" || this.state === "dying"
      ...
      : 0;
    this.fx.render(dt, this.fxLevel === 2 ? speedFx : 0, this.state === "dying", this.dieT / 0.9);
  }
